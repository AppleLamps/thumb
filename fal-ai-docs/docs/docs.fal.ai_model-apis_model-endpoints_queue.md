---
url: "https://docs.fal.ai/model-apis/model-endpoints/queue"
title: "Queue API | fal.ai Reference - fal"
---

[Skip to main content](https://docs.fal.ai/model-apis/model-endpoints/queue#content-area)
...
- [Queue endpoints](https://docs.fal.ai/model-apis/model-endpoints/queue#queue-endpoints)
- [Submit a request](https://docs.fal.ai/model-apis/model-endpoints/queue#submit-a-request)
- [Object Lifecycle](https://docs.fal.ai/model-apis/model-endpoints/queue#object-lifecycle)
- [Request status](https://docs.fal.ai/model-apis/model-endpoints/queue#request-status)
- [Status types](https://docs.fal.ai/model-apis/model-endpoints/queue#status-types)
- [Logs](https://docs.fal.ai/model-apis/model-endpoints/queue#logs)
- [Streaming status](https://docs.fal.ai/model-apis/model-endpoints/queue#streaming-status)
- [Cancelling a request](https://docs.fal.ai/model-apis/model-endpoints/queue#cancelling-a-request)
- [Getting the response](https://docs.fal.ai/model-apis/model-endpoints/queue#getting-the-response)
- [Using webhook callbacks](https://docs.fal.ai/model-apis/model-endpoints/queue#using-webhook-callbacks)
...
### [​](https://docs.fal.ai/model-apis/model-endpoints/queue\#queue-endpoints)  Queue endpoints
...
### [​](https://docs.fal.ai/model-apis/model-endpoints/queue\#submit-a-request)  Submit a request
...
### [​](https://docs.fal.ai/model-apis/model-endpoints/queue\#object-lifecycle)  Object Lifecycle
...
### [​](https://docs.fal.ai/model-apis/model-endpoints/queue\#request-status)  Request status
...
#### [​](https://docs.fal.ai/model-apis/model-endpoints/queue\#status-types)  Status types

Queue `status` can have one of the following types and their respective properties:

- **`IN_QUEUE`** (and status code 202):  - `queue_position`: The current position of the task in the queue.
  - `response_url`: The URL where the response will be available once the task is processed.
- **`IN_PROGRESS`** (and status code 202):  - `logs`: An array of logs related to the request. Note that it needs to be enabled, as explained in the next section.
  - `response_url`: The URL where the response will be available.
- **`COMPLETED`** (and status code 200):  - `logs`: An array of logs related to the request. Note that it needs to be enabled, as explained in the next section.
  - `response_url`: The URL where the response is available.

#### [​](https://docs.fal.ai/model-apis/model-endpoints/queue\#logs)  Logs
...
#### [​](https://docs.fal.ai/model-apis/model-endpoints/queue\#streaming-status)  Streaming status

If you want to keep track of the status of your request in real-time, you can use the streaming endpoint.
The response is `text/event-stream` and each event is a JSON object with the status of the request exactly as the non-stream endpoint.This endpoint will keep the connection open until the status of the request changes to `COMPLETED`.It supports the same `logs` query parameter as the status.
...
### [​](https://docs.fal.ai/model-apis/model-endpoints/queue\#cancelling-a-request)  Cancelling a request
...
Note that a request may still be executed after getting this response if it was very late in the queue process.If the request is already processed, you will get a `400 Bad Request` response with this body:
...
### [​](https://docs.fal.ai/model-apis/model-endpoints/queue\#getting-the-response)  Getting the response

Once you get the `COMPLETED` status, the `response` will be available along with its `logs`.The HTTP status code you get is typically the HTTP status code that was returned by the model. This way you can differentiate success (`200`) vs model error (`5XX`) vs invalid request (`4XX`). There are 2 exceptions:
...
### [​](https://docs.fal.ai/model-apis/model-endpoints/queue\#using-webhook-callbacks)  Using webhook callbacks

Instead of polling for the request status, you can have fal call a webhook when a request is finished. Please refer to the [Webhooks page](https://docs.fal.ai/model-apis/model-endpoints/webhooks).
...
\\
Previous](https://docs.fal.ai/model-apis/model-endpoints) [Webhooks API \| fal.ai Reference\\
\\
Next](https://docs.fal.ai/model-apis/model-endpoints/webhooks)
